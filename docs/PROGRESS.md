...
- [x] Документація перестала недораховувати власні гейти CI: `CONTRIBUTING.md`
      ...
      при рев'ю PR #78)
- [x] Перелік джоб і перевірок CI став перевірюваним: `check-config-paths.mjs`
      звіряє `ci.yml` із `package.json` і з документами, що інструктують
      запускати перевірки (`CONTRIBUTING.md`, PR-шаблон), в обидва боки. Пункт
      передбачив рецидив дослівно — додавання `check:toc` дало саме його, і
      знайшло це вже саме правило, а не людина. README натомість перестали
      перелічувати команди: огляду досить сказати, що CI звіряє документацію з
      кодом (див. `DECISIONS.md` → «Перевірка поверхні CI»)
- [x] Документацію стиснуто: секції Tooling і CI в `DECISIONS.md` зведені до
      оголошеного формату, `LEARNING.md` повернено до власного. Append-only
      знято, зміст у трьох довгих файлах генерує `scripts/generate-toc.mjs`
      під `npm run check:toc` у CI (див. `DECISIONS.md` → «Редакція
      документації»). Обсяг тут навмисно не числом: жодна перевірка його не
      звіряє, а попереднє формулювання встигло розійтись із реальністю
- [x] Спільний код перевірок винесено в `scripts/lib/report.mjs` — спрацював
      тригер «третій скрипт», записаний наперед у «Перевірка шляхів у конфігах
      тулінгу». Винесено лише те, що було байт-у-байт однакове (`formatCause`,
      `markFailed`); три `report` лишились на місці, бо розходяться текстом і
      сигнатурою, як той самий запис і передбачав
