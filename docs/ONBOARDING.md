...
налаштування. Окремо ставити нічого не треба, вона підхоплюється сама.

- `.claude/skills/` — `task-workflow` і `pr-workflow`. Підвантажуються, коли
  доречні (заводиш гілку, відкриваєш PR), а не в кожен запит.
- `.claude/hooks/doc-rules.cjs` — після правки файлу нагадує про документацію,
  ...
  `src/db/schema.ts`, на **новий** файл міграції або **новий** файл роута.
  На звичайну правку існуючого файлу мовчить.
