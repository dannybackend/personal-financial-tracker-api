...
кожен кореневий `*.config.*` присутній в `include` одного з двох tsconfig — і,
четверте, що читачі досі розуміють свої файли. Існує тому, що глоб, який не
збігається ні з чим — валідний YAML: інструкція CodeRabbit про валідацію env
два місяці вказувала на `src/config/**`, теку, якої ніколи не було, і жоден
інший сигнал про це не мовчав би.
