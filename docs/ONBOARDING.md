...
Звірити, що шляхи в конфігах тулінгу ще існують — окремий job `config-paths` у
CI. Ні токена, ні мережі не потребує, тому запускається як є:

```bash
npm run check:config-paths
```

Перевіряє чотири речі: кожен `path:` у `.coderabbit.yaml` збігається з реальним
файлом, кожен патерн у `knowledge_base.code_guidelines.filePatterns` теж,
кожен кореневий `*.config.*` реально входить у програму одного з двох tsconfig
(з урахуванням `files` і `exclude`, а не лише `include`) — і, четверте, що
читачі досі розуміють свої файли. Існує тому, що глоб, який не збігається ні з
чим — валідний YAML: інструкція CodeRabbit про валідацію env два місяці
вказувала на `src/config/**`, теку, якої ніколи не було, і жоден сигнал у
проєкті про це не сповістив.

Четверта перевірка — та, що спрацює найнесподіваніше. Читач `.coderabbit.yaml`
рядковий, не YAML-парсер, тож конфіг, переписаний у flow-стилі, прочитається
для нього порожнім. Тому `parsed 0 path instructions` означає не «все чисто», а
«скрипт більше не розуміє файл» — і валить збірку, замість тихо звітувати успіх
на нулі перевірених патернів.
