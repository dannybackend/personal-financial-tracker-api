...
// The hook's own tests live inside .claude/, so the blanket exclusion above
// silently dropped them from `npm test` while they still passed when run
// directly - a test that looks alive and never runs, which is precisely the
// failure the hook they cover exists to prevent. They exclude the worktree
// copies by name instead of the whole directory.
const hookExclude = [...configDefaults.exclude, '.claude/worktrees/**'];
