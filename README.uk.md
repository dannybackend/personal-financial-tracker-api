...
- CI на кожен pull request — typecheck, lint і весь набір тестів проти свіжого
  контейнера PostgreSQL (GitHub Actions)

**Далі**

Зміцнення схеми (інваріанти знака суми, календарні дати, soft delete), потім
категорії, транзакції й бюджети, потім агрегаційні ендпоінти — баланси,
витрати за категоріями, cashflow, виконання бюджету.
Живий беклог: [GitHub Issues](../../issues), згруповані в milestones.

Ще немає: деплою, OpenAPI-документації.
...
flowchart LR
    ...
    subgraph api["Hono API — Node.js 24 / TypeScript strict"]
        routes["Route handlers<br/>Zod → перевірка власності → логіка → типізована відповідь"]
...
| Мова | TypeScript 6, strict | Без `any`, явні типи повернення на експортах |
...
| CI | GitHub Actions | Typecheck, lint і тести на кожен PR, проти контейнера Postgres, зібраного з нуля щоразу |
...
- **Валюта — `char(3)` ISO-4217**, під захистом і `CHECK` у базі, і спільного
  Zod-enum, і живе на рахунку — транзакція успадковує її й ніколи не має
  власної. Будь-який агрегат, що сумує гроші більш ніж по одному рахунку,
  має групуватись по валюті й ніколи не змішувати. Валюта рахунку незмінна
  після створення.
...
| Чужий ресурс повертає `404`, ніколи `403` — але `422`, якщо його id прийшов у тілі запиту | `403` підтверджує, що ресурс існує; `404` не видає нічого. Id у тілі — це вхідні дані, тому він падає на валідації |
...
| Валюта живе на рахунку, `char(3)` ISO-4217, незмінна після створення | Транзакція успадковує валюту рахунку; єдина базова валюта на користувача не відповідає реальному користувачу цього трекера (картка UAH + депозит USD одночасно) |

Прийнято, але ще не реалізовано: тільки додатні суми з напрямом із `type`;
календарні дати транзакцій; soft delete рахунків; бюджети з явним
`period_start` і унікальністю в межах періоду; перекази як два зв'язані
